...
def ray_and_empty_voxel_overlap():
    for func in single_voxel_overlap:
        for xs in np.linspace(-1.0, 1.0, 3):
            for ys in np.linspace(-1.0, 1.0, 3):
                for zs in np.linspace(-1.0, 1.0, 3):
                    for xd in np.linspace(-1.0, 1.0, 3):
                        for yd in np.linspace(-1.0, 1.0, 3):
                            for zd in np.linspace(-1.0, 1.0, 3):
                                if xd != 0.0 or yd != 0.0 or zd != 0.0:
                                    ol = func(
...
def test_overlap_single_box_various_supports():
    for func in single_voxel_overlap:
        for dim in range(3):
            direction = np.zeros(3)
            direction[dim] = 1.0
            for xs in np.linspace(-0.5, 0.5, 3):
                for ys in np.linspace(-0.5, 0.5, 3):
                    for zs in np.linspace(-0.5, 0.5, 3):
                        ol = func(
                            support=np.array([xs, ys, zs]),
                            direction=direction,
                            xl=-1,
...
def test_overlap_in_octtree_space_edge_cases():
    for func in overlap_of_ray_with_voxels:
        ...
        ol = func(
            support=np.array([-1.0, 0.0, 0.0]),
            direction=np.array([0.0, 0.0, 1.0]),
            x_bin_edges=np.linspace(-1.0, 1.0, 3),
            y_bin_edges=np.linspace(-1.0, 1.0, 3),
            z_bin_edges=np.linspace(-1.0, 1.0, 3),
        )
        assert len(ol["x"]) == 2
        assert len(ol["y"]) == 2
        assert len(ol["z"]) == 2
...
def test_non_straight_overlaps():
    for func in overlap_of_ray_with_voxels:
        ...
        assert ol["z"][1] == 1
        assert (
            np.abs(ol["overlap"][0] - np.linalg.norm(np.array([1, 1, 1])))
            < 1e-6
        )
        assert (
            np.abs(ol["overlap"][1] - np.linalg.norm(np.array([1, 1, 1])))
            < 1e-6
        )
...
def test_non_linear_voxel_space():
    for func in overlap_of_ray_with_voxels:
        ol = func(
            support=np.array([0.0, 0.1, 0.0]),
            direction=np.array([0.1, 0.5, 1.0]),
            x_bin_edges=np.logspace(0.0, 1.0, 10),
            y_bin_edges=np.logspace(0.0, 1.0, 10),
            z_bin_edges=np.logspace(0.0, 1.0, 10),
        )
...
def test_estimate_overlap_of_ray_bundle_with_voxels():
    ...
    supports = np.array(
        [
            np.random.normal(loc=0.0, scale=0.2, size=num_rays_in_bundle),
            np.random.normal(loc=0.0, scale=0.2, size=num_rays_in_bundle),
    ...
    z_bin_edges = np.linspace(0.0, edge_length, 3)
...
def test_tomographic_system_matrix():
    ...
    psf = rvo.estimate_system_matrix(
        supports=supports,
        directions=directions,
        x_bin_edges=np.linspace(-100.0, 100.0, N_X_BINS + 1),
        y_bin_edges=np.linspace(-100.0, 100.0, N_Y_BINS + 1),
        z_bin_edges=np.linspace(0.0, 200.0, N_Z_BINS + 1),
    )
